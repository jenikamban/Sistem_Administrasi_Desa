                ...
                <div class="row g-3">
                    <div class="col-md-12">
                        ...
                        <select class="form-select @error('warga_id') is-invalid @enderror" id="warga_id" name="warga_id" required>
                            ...
                            @foreach ($wargas as $w)
                                <option value="{{ $w->id }}" {{ old('warga_id', request('warga_id')) == $w->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('jenis_mutasi') is-invalid @enderror" id="jenis_mutasi" name="jenis_mutasi" required>
                            ...
                            <option value="Keluar" {{ old('jenis_mutasi') == 'Keluar' ? 'selected' : '' }}>Keluar (Pindah)</option>
                        </select>
                        @error('jenis_mutasi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan" rows="3" required placeholder="Contoh: Pindah ke alamat baru di kota lain, atau sebab kematian">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
